...
for (i in 1:49) {
  for (j in 1:261) {
    print(1)
    amazon_state_name[i, paste(amazon_trend[j, "Week"])] <- 
      gtrends("amazon", geo = paste("US", amazon_state[i, "Abbreviation"], sep = "-"), 
              gprop = "web", 
              time = paste(paste(amazon_trend[j, "Week"]), 
                           as.Date(paste(amazon_trend[j, "Week"]), 
                                   format = "%Y-%m-%d") + days(7)))$interest_over_time[j, "hits"] * 
    amazon_trend[j, "Amazon.com: (United States)"]
    print(2)
  }
}
...
for (i in 1:49) {
  ...
  data <- mutate(data, date = paste(as.Date(date, format = "%m-%d-%Y"))) %>% 
    select("date", "hits") 
  names(data)[2] <- paste(amazon_state_name[i, "Abbreviation"])
